...
data = torch.tensor(dataset.encode(dataset.text), dtype=torch.long)
n = int(0.9*len(data))
train_data = data[:n]
val_data = data[n:]
...
config = BLMConfig(vocab_size=dataset.vocab_size, device="mps" if torch.backends.mps.is_available() else "cpu")
